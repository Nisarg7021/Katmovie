    ...
    if 2 < len(message.text) < 100:    
        ...
        if files:
            for file in files:
                ...
                btn.append(
                    [InlineKeyboardButton(text=f"{filename}",callback_data=f"shinukat#{file_id}")]
                    )
        else:
        ...
        if len(btn) > 10: 
            ...
            if poster:
                await message.reply_photo(photo=poster, caption=shinukat, reply_markup=InlineKeyboardMarkup(buttons))

            else:
                await message.reply_text(shinukat, reply_markup=InlineKeyboardMarkup(buttons))
            return
    ...
    if 2 < len(message.text) < 50:    
        ...
        if files:
            for file in files:
                ...
                filename = f"[{get_size(file.file_size)}] {file.file_name}"
                btn.append(
                    [InlineKeyboardButton(text=f"{filename}", url=f"https://telegram.dog/{nyva}?start=shinu_-_-_-_{file_id}")]
                )
        else:
            return
        if not btn:
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if (clicked == typed):
        ...
        elif query.data.startswith("shinukat"):
